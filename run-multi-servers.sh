...
FAULTS="${TUSK_FAULTS:-0}"
ADVERSARY_SEED="${TUSK_ADVERSARY_SEED:-0}"
CLIENT_DURING_SILENCE="${TUSK_CLIENT_DURING_SILENCE:-pause}"
...
CLIENT_SILENCE_SLOT_MS="${TUSK_CLIENT_SILENCE_SLOT_MS:-}"
...
[[ "$CLIENT_SILENCE_SLOT_MS" =~ ^[1-9][0-9]*$ ]] || { echo "TUSK_CLIENT_SILENCE_SLOT_MS must be positive" >&2; exit 2; }
mapfile -t CLIENT_SCHEDULES < <(
  TUSK_CLIENT_DURING_SILENCE="$CLIENT_DURING_SILENCE" \
  TUSK_ADVERSARY_SEED="$ADVERSARY_SEED" \
...
for i in "${!IPS[@]}"; do remote "${IPS[$i]}" "cd '$REMOTE_DIR' && tmux new-session -d -s tusk-primary \"RUST_LOG=info TUSK_FAULTS='$FAULTS' TUSK_ADVERSARY_SEED='$ADVERSARY_SEED' TUSK_CLIENT_DURING_SILENCE='$CLIENT_DURING_SILENCE' ./target/release/node -vv run --keys deploy/node-${i}.json --committee deploy/committee.json --parameters deploy/parameters.json --store run/db-primary primary |& tee run/logs/primary-${i}.log\""; done
...
for i in "${!IPS[@]}"; do
  ...
  remote "${IPS[$i]}" "cd '$REMOTE_DIR' && tmux new-session -d -s tusk-client \"RUST_LOG=info ./target/release/benchmark_client '${IPS[$i]}:3003' --size '$TX_SIZE' --rate '$RATE_SHARE' $SILENCE_ARGS --nodes $TX_NODES |& tee run/logs/client-${i}-0.log\""
done
...
cd benchmark
TUSK_RESULT_FAULTS="$FAULTS" python3 - <<'PY'
import os
from benchmark.logs import LogParser
print(LogParser.process('logs', faults=int(os.environ['TUSK_RESULT_FAULTS'])).result())
PY
