...
```
They specify the number of primaries (`nodes`) and workers per primary (`workers`) to deploy, the input rate (tx/s) at which the clients submit transactions (`rate`), the transaction size (`tx_size`), the dynamic adversary count (`faults`), and the duration. With `faults > 0`, all nodes remain online and derive the same pseudo-random set of `f` silent authorities in every round from `TUSK_ADVERSARY_SEED`. A silent primary suppresses its Header while continuing to receive messages. Before startup, the benchmark generates one wall-clock silence bitmap per client. `TUSK_CLIENT_DURING_SILENCE=pause` (default) stops client transactions and local batch production in silent slots; `send` preserves input and batch traffic. Slots default to `max_header_delay` and can be overridden with `TUSK_CLIENT_SILENCE_SLOT_MS`. Direct commits are scheduled in windows of `3f+1` leader rounds: after excluding silent leaders, up to `f+2` slots are selected by a deterministic hash, targeting a `(f+2)/(3f+1)` direct-commit ratio.
